...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self) {
        ...
        self.image = [NSImage imageNamed:@"Add-File"];
...
-(NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender{
    ...
    NSString* filePath = [[[sender draggingPasteboard] propertyListForType:NSFilenamesPboardType] lastObject];
    NSLog(@"filePath --- %@",filePath);
    if ([[pboard types] containsObject:NSFilenamesPboardType]) {
//        if ([[[filePath substringFromIndex:[filePath length] - 10] lowercaseString] isEqualToString:@".xcodeproj"]) {
//            self.image =  [NSImage imageNamed:@"xcode"];
//
